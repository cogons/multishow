...
void multishow(const std::string& _winName, const vector<Mat>& _img)
{
	int nImg = (int)_img.size();
	
	vector<cv::Mat> _img3;

	for (int i = 0; i < nImg; i++){
		Mat _img3Mat;
		if (_img[i].channels() == 1){
			cvtColor(_img[i], _img3Mat, CV_GRAY2BGR);
			_img3.push_back(_img3Mat);
		}
	}
	...
	for (int i = 0, m = 20, n = 20; i<nImg; i++, m += (20 + size))
	{
		...
		resize(_img3[i], imgROI, Size((int)(x / scale), (int)(y / scale)));
	}
